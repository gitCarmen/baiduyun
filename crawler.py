                    ...
                    if "total_count" in follows.keys() and follows["total_count"] > 0 and str(start) == "0":  # 获得订阅数量
                        ...
                        for i in range((follows["total_count"] - 1) / ONEPAGE):  # 开始一页一页获取有用信息
                            ...
                            try:
                                ...
                                dbcurr.execute(
                                    'INSERT INTO urlids(uk, start, limited, type, status) VALUES(%s, %s, %s, 1, 0)' % (
                                    uid, str(ONEPAGE * (i + 1)), str(ONEPAGE)))
                    ...
                    if "follow_list" in follows.keys():  # 如果订阅者也订阅了，即拥有follow_list
                        ...
                        for item in follows["follow_list"]:
                            ...
                            try:
                                ...
                                dbcurr.execute(
                                    'INSERT INTO user(userid, username, files, status, downloaded, lastaccess) VALUES(%s, "%s", 0, 0, 0, %s)' % (
                                    item['follow_uk'], item['follow_uname'], str(tnow)))
                    ...
                    if "total_count" in fans.keys() and fans["total_count"] > 0 and str(start) == "0":
                        ...
                        for i in range((fans["total_count"] - 1) / ONEPAGE):
                            ...
                            try:
                                ...
                                dbcurr.execute(
                                    'INSERT INTO urlids(uk, start, limited, type, status) VALUES(%s, %s, %s, 2, 0)' % (
                                    uid, str(ONEPAGE * (i + 1)), str(ONEPAGE)))
                    ...
                    if "fans_list" in fans.keys():
                        ...
                        for item in fans["fans_list"]:
                            ...
                            try:
                                ...
                                dbcurr.execute(
                                    'INSERT INTO user(userid, username, files, status, downloaded, lastaccess) VALUES(%s, "%s", 0, 0, 0, %s)' % (
                                    item['fans_uk'], item['fans_uname'], str(tnow)))
                    ...
                    if "total_count" in shares.keys() and shares["total_count"] > 0 and str(start) == "0":
                        ...
                        for i in range((shares["total_count"] - 1) / ONESHAREPAGE):
                            ...
                            try:
                                ...
                                dbcurr.execute(
                                    'INSERT INTO urlids(uk, start, limited, type, status) VALUES(%s, %s, %s, 0, 0)' % (
                                    uid, str(ONESHAREPAGE * (i + 1)), str(ONESHAREPAGE)))
                    ...
                    if "records" in shares.keys():
                        ...
                        for item in shares["records"]:
                            ...
                            try:
                                ...
                                dbcurr.execute(
                                    'INSERT INTO share(userid, filename, shareid, status) VALUES(%s, "%s", %s, 0)' % (
                                    uid, item['title'], item['shareid']))  # item['title']恰好是文件名称
            ...
            if d:
                ...
                for item in d:
                    ...
                    try:
                        ...
                        dbcurr.execute(
                            'insert into urlids(uk, start, limited, type, status) values("%s", 0, %s, 0, 0)' % (
                            item[1], str(ONESHAREPAGE)))
                        ...
                        dbcurr.execute(
                            'insert into urlids(uk, start, limited, type, status) values("%s", 0, %s, 1, 0)' % (
                            item[1], str(ONEPAGE)))

                        dbcurr.execute(
                            'insert into urlids(uk, start, limited, type, status) values("%s", 0, %s, 2, 0)' % (
                            item[1], str(ONEPAGE)))
